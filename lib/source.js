...
function parse(source, fileName) {
  ...
  function saveEntry(entry) {
    ...
    data.push({ name, fileName, ln: line, props });
  }
      ...
      case ts.SyntaxKind.JsxSelfClosingElement:
        ...
        if (name && /[A-Z]/.test(name[0])) {
          ...
          props.forEach(prop => {
            ...
            toSave.push({ value, name: propName, spread });
          });

          toSave.push(...(spread ? [{ name: '...spread', value: true }] : []))
          saveEntry({ node, name, props: toSave });
        }
...
module.exports = function(config) {
  glob(config.cwd + config.include, {}, function(er, files) {
  ...
  console.log('👍  Sourced Successfully')
}