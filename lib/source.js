...
module.exports = function(config, callback) {
  glob(config.cwd + config.include, {}, function(err, files) {
    ...
    console.log(chalk.white(`Propspot reading ${files.length} files...`));
    ...
    const dir = `${config.cwd}/node_modules/propspot/.propspotcache`;
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir);
    }
    fs.writeFileSync(dir + '/data.json', JSON.stringify(data));

    console.log(chalk.green('Propspot source done'));
  });
};
